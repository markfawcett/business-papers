...
import Python_Resources.createOP_App_cmd as cmd_vsn
...
class OPApp:
    def __init__(self, master):
        ...
        self.temp_folder_button = ttk.Button(self.frame, text="Last opdx.xml file", width=20, command=self.get_opds_file)
        ...
        # self.temp_folder_button.grid(row=2, column=0, stick='w', padx=10, pady=3)


        # EB text entry lable
        self.EB_box_lable = ttk.Label(self.frame, text='OP Number:  ')
        self.EB_box_lable.grid(row=3, column=0, stick='w', pady=3)
        # EB text entry box
        self.EB_box = ttk.Entry(self.frame, textvariable=self.OP_number, width=4)
        self.EB_box.grid(row=3, column=1, stick='w', pady=3)

        # Sitting date lable
        self.Sitting_box_lable = ttk.Label(self.frame, text='Sitting Date\t(e.g. 2016-09-12):  ')
        self.Sitting_box_lable.grid(row=4, column=0, stick='w', pady=3)
        # Sitting date box
        self.Sitting_box = ttk.Entry(self.frame, textvariable=self.sitting_date, width=11)
        self.Sitting_box.grid(row=4, column=1, stick='w', pady=3)

        self.run_OP_tool_button = ttk.Button(self.frame, text="Run", width=12, command=self.run_OP_tool)
        self.run_OP_tool_button.grid(row=5, column=0, columnspan=3, padx=10, pady=10)


    def run_OP_tool(self):

        infilename   = self.input_file_path.get()
        opds_file    = self.opds_file_path.get()
        # temp_folder  = self.temp_folder_path.get()
        sitting_date = self.sitting_date.get()
        op_number    = self.OP_number.get()

        # run functions
        # cmd_vsn.create_app(infilename, temp_folder, opds_file, sitting_date, op_number)
        cmd_vsn.create_app(infilename, opds_file, sitting_date, op_number)
    ...
    def get_opds_file(self):
        directory = filedialog.askopenfilename()
        self.opds_file_path.set(directory)
        print('opds file is at:  ' + self.opds_file_path.get())
