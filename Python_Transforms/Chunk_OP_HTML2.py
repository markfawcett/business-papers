...
class OPApp:
    def __init__(self, master):
        ...
        # Creation date box
        Creation_box = ttk.Entry(self.frame_top, textvariable=self.creation_date, width=11)
        ...
        # Sitting date box
        Sitting_box = ttk.Entry(self.frame_top, textvariable=self.sitting_date, width=11)
    ...
    def run_OP_tool(self):
        ...
        cmd_version.DATES.set_up(creation_date, sitting_date)
        ...
        # also download XML from the Order Paper portal
        # this is to publish to publications.parliament
        # it is uncorrected but useful still to people like
        # volify
        try:
            import Python_Resources.getUncorrectedXMLfromPortal as xml_from_portal
        except ImportError:
            print('Error, Unable to get getUncorrectedXMLfromPortal.py file.',
                  'XML for publications.parliament will not be created.')
        else:
            output_folder = os.path.dirname(infilename)
            xml_from_portal.get_xml_from_portal_to_publish(sitting_date, output_folder)
